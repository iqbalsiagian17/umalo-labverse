                ...
                <div class="table-responsive">
                    <table class="table table-bordered">
                        ...
                        <tbody>
                            ...
                            @if (in_array($order->status, ['Diterima', 'Packing', 'Pengiriman', 'Selesai']) && $order->orderItems->contains(function ($item) {
                                ...
                                <tr>
                                    <td colspan="3" class="text-right">
                                        <strong>{{ __('messages.subtotal_before_negotiation') }}</strong>
                                    </td>
                                    <td class="text-right">
                                        <strong>{{ 'Rp ' .
                                            number_format(
                            ...
                            @if (in_array($order->status, ['Diterima', 'Packing', 'Pengiriman', 'Selesai']) && $order->harga_setelah_nego)
                                <!-- Harga Setelah Nego -->
                                <tr>
                                    <td colspan="3" class="text-right">
                                        <strong>{{ __('messages.negotiated_price') }}</strong>
                                    </td>
                                    <td class="text-right">
                    ...
                    <p class="text-muted small">
                        <span class="text-danger">*</span>Harga yang tertera belum termasuk pajak PPN. Untuk mengetahui
                        total transaksi yang sebenarnya, silakan melihat pada faktur (invoice) yang dapat diunduh.
                    </p>
                </div>
                ...
                <div class="mt-4">
                    @if (
                        ...
                        @if ($order->status == 'Negosiasi' && $order->whatsapp_number)
                            <div class="alert alert-info">
                                <strong>{{ __('messages.whatsapp_number_for_negotiation') }}:</strong>
                                {{ $order->whatsapp_number }}<br>
                    ...
                    @if (in_array($order->status, [
                            ...
                            'Menunggu Konfirmasi Admin untuk Negosiasi',
                            'Negosiasi',
                            'Diterima',
                        ]))
                            ...
                            <div class="card-body">
                                ...
                                <form action="{{ route('order.upload_bukti_pembayaran', $order->id) }}" method="POST" enctype="multipart/form-data">
                                    ...
                                    <div class="alert alert-warning mt-3 d-flex align-items-center">
                                        ...
                                        <p class="mb-0">
                                            Untuk mengetahui jumlah pembayaran yang harus Anda lakukan, silakan lihat pada
                                            faktur yang dapat diunduh melalui tombol 'Download Invoice' di bagian atas
                                            halaman ini. Terima kasih atas perhatian Anda.
                                        </p>
                                    </div>
                        ...
                        <div class="card mt-4">
                            ...
                            <div class="card-body">
                                ...
                                <a href="{{ asset('uploads/bukti_pembayaran/' . $order->bukti_pembayaran) }}"
                                    target="_blank" class="btn btn-info">{{ __('messages.view_payment_proof') }}</a>
                            </div>
                        </div>
                    @endif
                </div>
                                ...
                                @foreach ($order->statusHistories as $history)
                                    <li class="timeline-item {{ $loop->first ? 'timeline-item-first' : '' }}">
                                        ...
                                        <div class="timeline-content">
                                            <span
                                                class="timeline-date">{{ $history->created_at->format('d-m-Y H:i') }}</span>
                                            <h5 class="timeline-status">
                                                {{ __('messages.' . strtolower(str_replace(' ', '_', $history->status))) }}
                                            </h5>
